...
def model(in_file):
    ...
    ques1_enc.add(LSTM(HIDDEN_DIM, input_length=seq_maxlen, return_sequences=False))
    ques1_enc.add(Dropout(0.3))
    
    ques2_enc = Sequential()
    ques2_enc.add(Embedding(output_dim=EMBED_DIM, input_dim=vocab_size,
                       weights=[embedding_weights], mask_zero=True))
    ques2_enc.add(LSTM(HIDDEN_DIM, input_length=seq_maxlen, return_sequences=False))
...
if __name__ == '__main__':
    model("../data/quora_duplicate_questions.tsv.10000")
